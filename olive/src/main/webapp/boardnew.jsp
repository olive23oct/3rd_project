   ...
   $('#boardTitleChangeBtn').click(function() {
	   ...
	   //카드추가가 클릭됐을때
	    $(document).on("submit", ".addCardForm", function (e) {
	       e.preventDefault();
	       let addCardLabel = $(this).parent().children(".addCardLabel");
	       let addCardContent = $(this).children(".addCardContent");
	       let CardContent = addCardContent.val();
	       if(addCardContent.val().length>0){
	   ...
	   //카드추가 버튼이 실행됐을때
	    $(document).on("click", ".addCardButton", function (e) {
	       e.preventDefault();
	       $(this).parent(".addCardForm").submit();
	    });
	               ...
	               `<div class="list">
	                   ...
	                   <div class="cardAddWrap">
	                      ...
	                      <form class="addCardForm">
	                         <textarea name="" class="addCardContent" cols="30" rows="10" placeholder="카드내용을 입력하세요"></textarea>
						...
						<div class="list">
							<div class="listTitleWrap">
								...
								<form id="listTitleForm" action="updateTitle.do">
									<input class="listTitleChangedValue" type="text" name="title"
										placeholder="리스트이름 변경" autocomplete="off"> <input
							...
							<c:forEach var="card" items="${boardContents}"
								...
								<c:if test="${not loop_flag }">
									...
									<c:if test="${card.card_id != null}">
										<div class="cardWrap">
											<div class="cardContent">${card.title}</div>
										</div>
									</c:if>
								</c:if>
							</c:forEach>
							<div class="cardAddWrap">
								...
								<div class="addCardLabel">+ 카드 추가</div>
								<form class="addCardForm">
									<textarea id="addCardTitle" name="title" class="addCardContent"
										cols="30" rows="10" placeholder="카드내용을 입력하세요"></textarea>
									...
									<input type="button" class="addCardButton" id="addCardTitle"
										onclick="getCardData()">카드 추가 <span class="close"
